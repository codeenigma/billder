    ...
    billing_data.get_billing_data(account.AWS, true, function(costs) {
      ...
      for (var product in costs.products) {
        ...
        if (config.provider.incomeCategory) {
          invoice_item['category'] = config.provider.auth.tokenHost + '/v2/categories/' + config.provider.incomeCategory;
        }
        invoice_items.push(invoice_item);
      }
      ...
      var invoice = {
        'invoice' : {
          'status'                : 'Draft',
          'contact'               : config.provider.auth.tokenHost + '/v2/contacts/' + account.FAC,
      ...
      if (account.FAP) {
        invoice['invoice']['project'] = config.provider.auth.tokenHost + '/v2/projects/' + account.FAP;
      }
      // Handle EU VAT
      if (account.EU) {
...
function send_invoice(account, invoice) {
    options = api.build_request_options(config.provider.auth.tokenHost, '/v2/invoices', '/tmp/fa-access-token.txt', 'POST', invoice);
    request(options)
