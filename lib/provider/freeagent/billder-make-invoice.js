...
module.exports = {
   ...
   * @param {String} month, target month in 'YYYY-DD' format
  ...
  make_invoice(month, config, account) {
     ...
     */
    if (config.aws.organization) {
      linkedAccountId = account.AWS;
      console.log('Billder: account uses AWS Organizations, supplying linked account ID');
    }
    else {
      linkedAccountId = '';
      console.log('Billder: account does not use AWS Organizations, not sending linked account ID');
    }
    billing_data.get_billing_data(config, month, linkedAccountId, true, function(costs) {
      var invoice_items = [];
      var running_total = 0;
      var products = [];
      console.log(costs);
