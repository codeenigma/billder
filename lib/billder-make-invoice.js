  ...
  request(options)
    .then(function (response) {
      ...
      billing_data.get_billing_data(account.AWS, true, function(costs) {
         ...
         */
        options = api.build_request_options(endpoint=config.auth.tokenHost,path='/v2/invoices', access_token_file='/tmp/fa-access-token.txt', request_method='POST', data=invoice);
        request(options)
          .then(function (response) {
